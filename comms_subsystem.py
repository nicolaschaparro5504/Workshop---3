class Communication_Subsystem:
    ...
    def send_status(self, status, skip_summary=False):
        ...
        print(f"[Comms] Transmiting: {status}")
   
        if len(self.sent_chars) >= self._messages_before_summary and not skip_summary: # Condition to summarize
            self.summarize()
    ...
    def summarize(self):
        ...
        estimated_consumption = 0.05 * total_chars # Counts all the characters and multiplies it by 0.05
